...
const EmployeeManagement = () => {
  ...
  const handleEditEmployee = (employee) => {
    alert(
      ...
      `Chức vụ: ${employee.role === 'TELLER' ? 'Giao dịch viên' : 'Nhân viên Tín dụng'}\n\n` +
  ...
  const handleDeleteEmployee = (employee) => {
    if (confirm(`Bạn có chắc chắn muốn xóa nhân viên ${employee.fullName}?`)) {
      alert(
        `🗑️ Đã xóa nhân viên ${employee.fullName}\n\n` +
        `Mã NV: ${employee.employeeCode}\n` +
  ...
  const handleResetPassword = (employee) => {
    if (confirm(`Bạn có muốn reset mật khẩu cho nhân viên ${employee.fullName}?`)) {
          ...
          <div className="stat-header">
            <div>
              ...
              <div className="stat-value">
                {employees.filter(e => e.role === 'TELLER').length}
              </div>
            </div>
            <User size={40} className="stat-icon" />
          </div>
        ...
        <div className="stat-card warning">
          <div className="stat-header">
            <div>
              ...
              <div className="stat-value">
                {employees.filter(e => e.role === 'LOAN_OFFICER').length}
              </div>
            </div>
            <User size={40} className="stat-icon" />
          </div>
      ...
      <div className="card">
        ...
        <table className="table">
          ...
          <tbody>
            {employees.map((emp) => (
              <tr key={emp.id}>
                <td className="font-medium">{emp.employeeCode}</td>
                <td>
                ...
                <td>{emp.branchCode || 'HN001'}</td>
                <td>
                  <span className={`badge ${emp.role === 'TELLER' ? 'badge-info' : 'badge-warning'}`}>
                    {emp.role === 'TELLER' ? 'Giao dịch viên' : 'Nhân viên Tín dụng'}
                  </span>
                </td>
                <td>
