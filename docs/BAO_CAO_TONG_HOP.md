...
5. THIẾT KẾ CƠ SỞ DỮ LIỆU
   ...
   5.4. Ràng buộc và rủi ro dữ liệu

6. XÂY DỰNG MẪU THỬ
   6.1. Môi trường phát triển
   6.2. Các module đã triển khai
   6.3. Giao diện người dùng
   6.4. Kết quả thử nghiệm
...
- Các cột chính: branch_id (PK), branch_code (UNIQUE), branch_name, address, phone, email, manager_id (FK → Employees), status, created_at
- Quan hệ: 1-n với Employees, 1-n với Customers, 1-n với Accounts

Bảng Roles (Vai trò)
- Mục đích: Định nghĩa các vai trò trong hệ thống
- Các cột chính: role_id (PK), role_name (UNIQUE), description, created_at
- Giá trị: CUSTOMER, TELLER, LOAN_OFFICER, ADMIN
- Quan hệ: 1-n với Employees

Bảng Employees (Nhân viên)
- Mục đích: Quản lý thông tin nhân viên ngân hàng
- Các cột chính: employee_id (PK), employee_code (UNIQUE), branch_id (FK), role_id (FK), full_name, email (UNIQUE), phone, username (UNIQUE), password_hash, status, created_at
...
- Các cột chính: customer_id (PK), customer_code (UNIQUE), full_name, email (UNIQUE), phone (UNIQUE), id_card_number, date_of_birth, gender, address, occupation, monthly_income, registered_branch_id (FK), username (UNIQUE), password_hash, status, created_at
...
- Các cột chính: score_id (PK), customer_id (FK, UNIQUE), score (300-850), grade (AAA-C), interest_rate, max_ltv, on_time_payment_rate, debt_to_income_ratio, account_age_months, total_outstanding_debt, late_payments, monthly_income, last_updated
- Quan hệ: 1-1 với Customers

Bảng CreditScoreHistory (Lịch sử điểm tín dụng) - Bảng phụ
- Mục đích: Ghi lại mọi thay đổi điểm tín dụng
- Các cột chính: history_id (PK), customer_id (FK), old_score, new_score, score_change, old_grade, new_grade, reason, event_type, event_reference, created_by (FK), created_at
...
- Các cột chính: account_type_id (PK), type_code (UNIQUE), type_name, description, min_balance, interest_rate, status
- Quan hệ: 1-n với Accounts

Bảng Accounts (Tài khoản)
- Mục đích: Quản lý tài khoản của khách hàng
- Các cột chính: account_id (PK), account_number (UNIQUE), customer_id (FK), account_type_id (FK), branch_id (FK), balance, interest_rate, opened_date, status, created_at
...
- Các cột chính: card_id (PK), card_number (UNIQUE, ENCRYPTED), account_id (FK), card_type (DEBIT/CREDIT), issue_date, expiry_date, status, created_at
...
- Các cột chính: transaction_type_id (PK), type_code (UNIQUE), type_name, description
- Quan hệ: 1-n với Transactions

Bảng Transactions (Giao dịch)
- Mục đích: Ghi nhận mọi giao dịch trong hệ thống
- Các cột chính: transaction_id (PK, BIGINT), transaction_code (UNIQUE), transaction_type_id (FK), from_account_id (FK), to_account_id (FK), from_bank_name, to_bank_name, amount, fee, description, balance_after, status, transaction_date
...
- Các cột chính: rate_id (PK), term_months, min_amount, max_amount, interest_rate, status, effective_from, effective_to
- Quan hệ: 1-n với SavingsDeposits

Bảng SavingsDeposits (Sổ tiết kiệm)
- Mục đích: Quản lý sổ tiết kiệm của khách hàng
- Các cột chính: deposit_id (PK), deposit_number (UNIQUE), customer_id (FK), account_id (FK), branch_id (FK), principal_amount, interest_rate_id (FK), interest_rate, term_months, start_date, maturity_date, auto_renew, total_interest_earned, status, created_at
...
- Các cột chính: savings_transaction_id (PK), deposit_id (FK), transaction_type, amount, interest_amount, description, transaction_date
- Quan hệ: n-1 với SavingsDeposits

 5.2.6. Nhóm bảng Vay vốn

Bảng CollateralTypes (Loại tài sản thế chấp)
- Mục đích: Định nghĩa loại tài sản thế chấp
- Các cột chính: collateral_type_id (PK), type_code (UNIQUE), type_name, max_ltv
- Quan hệ: 1-n với Collaterals

Bảng Collaterals (Tài sản thế chấp)
- Mục đích: Quản lý tài sản thế chấp của khách hàng
- Các cột chính: collateral_id (PK), customer_id (FK), collateral_type_id (FK), collateral_name, description, location, certificate_number, issue_date, issue_authority, original_value, appraised_value, appraised_by (FK), appraised_date, status, verification_notes, created_at
- Quan hệ: n-1 với Customers, n-1 với CollateralTypes, n-1 với Employees, 1-n với Loans

Bảng LoanTypes (Loại hình vay)
- Mục đích: Định nghĩa các loại hình vay
- Các cột chính: loan_type_id (PK), type_code (UNIQUE), type_name, description, max_amount, max_term_months, require_collateral
- Quan hệ: 1-n với Loans, 1-n với LoanInterestRates

Bảng LoanInterestRates (Lãi suất cho vay)
- Mục đích: Cấu hình lãi suất theo loại vay, hạng tín dụng
- Các cột chính: rate_id (PK), loan_type_id (FK), min_amount, max_amount, term_months_from, term_months_to, credit_grade, base_rate, effective_from, effective_to, status
- Quan hệ: n-1 với LoanTypes, 1-n với Loans

Bảng Loans (Khoản vay)
- Mục đích: Quản lý các khoản vay của khách hàng
- Các cột chính: loan_id (PK), loan_number (UNIQUE), customer_id (FK), account_id (FK), loan_type_id (FK), branch_id (FK), loan_officer_id (FK), loan_amount, approved_amount, interest_rate_id (FK), interest_rate, term_months, monthly_payment, purpose, collateral_id (FK), collateral_value, ltv_ratio, outstanding_balance, application_date, approved_date, approved_by (FK), rejection_reason, disbursement_date, first_payment_date, maturity_date, status, created_at
- Quan hệ: n-1 với Customers, n-1 với Accounts, n-1 với LoanTypes, n-1 với Branches, n-1 với Employees (loan_officer, approved_by), n-1 với LoanInterestRates, n-1 với Collaterals, 1-n với LoanPaymentSchedule, 1-n với LoanPayments

Bảng LoanPaymentSchedule (Lịch trả nợ)
- Mục đích: Lưu lịch trả nợ định kỳ cho từng khoản vay
- Các cột chính: schedule_id (PK), loan_id (FK), payment_number, due_date, principal_amount, interest_amount, total_payment, outstanding_balance, status, paid_amount, paid_date
- Quan hệ: n-1 với Loans, 1-n với LoanPayments

Bảng LoanPayments (Thanh toán khoản vay) - Bảng phụ
- Mục đích: Ghi nhận từng lần thanh toán khoản vay
- Các cột chính: payment_id (PK), loan_id (FK), schedule_id (FK), transaction_id (FK), payment_date, principal_paid, interest_paid, late_fee_paid, total_paid, payment_method, reference_number, notes
- Quan hệ: n-1 với Loans, n-1 với LoanPaymentSchedule, n-1 với Transactions

 5.3. Quan hệ giữa các bảng

Các quan hệ chính:
- Branches ↔ Employees: 1-n (một chi nhánh có nhiều nhân viên)
- Customers ↔ Accounts: 1-n (một khách hàng có nhiều tài khoản)
- Accounts ↔ Transactions: 1-n (một tài khoản có nhiều giao dịch)
- Customers ↔ Loans: 1-n (một khách hàng có nhiều khoản vay)
- Loans ↔ LoanPaymentSchedule: 1-n (một khoản vay có nhiều kỳ trả nợ)
- Customers ↔ SavingsDeposits: 1-n (một khách hàng có nhiều sổ tiết kiệm)
- Customers ↔ Collaterals: 1-n (một khách hàng có nhiều tài sản thế chấp)

 5.4. Ràng buộc và rủi ro dữ liệu

Ràng buộc dữ liệu:
- Foreign Key Constraints: Đảm bảo tính toàn vẹn tham chiếu
- UNIQUE Constraints: Ngăn trùng lặp (account_number, loan_number, email, username)
- CHECK Constraints: Ràng buộc giá trị (amount > 0, score 300-850)
- NOT NULL: Các trường bắt buộc

Bảo vệ dữ liệu:
- Mã hóa thông tin nhạy cảm (card_number, password_hash)
- Indexes tối ưu cho các truy vấn thường xuyên
- Triggers tự động cập nhật số dư, điểm tín dụng
- Stored Procedures đóng gói logic nghiệp vụ phức tạp
...
- Thư viện: React Router DOM, Recharts, Lucide React
...
- Engine: InnoDB
- Character Set: utf8mb4
- Schema: 21 bảng (19 chính + 2 phụ)

Backend:
- Hiện tại: Mock data (frontend-only)
- Dự kiến: Spring Boot hoặc Node.js + Express
- API: RESTful API với JWT authentication

 6.2. Các module đã triển khai

Module Khách hàng (Customer Portal):
- Dashboard tổng quan với biểu đồ
- Quản lý tài khoản và giao dịch
- Gửi tiết kiệm (3 bước)
- Đăng ký vay (3 bước với tính LTV)
- Quản lý tài sản và điểm tín dụng

Module Nhân viên Giao dịch (Teller Portal):
- Mở tài khoản mới (3 bước)
- Nạp tiền tại quầy (3 bước)
- Rút tiền tại quầy (3 bước)
- Kích hoạt thẻ (3 bước)

Module Nhân viên Tín dụng (Loan Officer Portal):
- Dashboard phân loại hồ sơ
- Thẩm định vay (4 bước)
- Quản lý nợ quá hạn
- Xét duyệt tài sản thế chấp

Module Quản trị viên (Admin Portal):
- Dashboard thống kê toàn hệ thống
- Quản lý chi nhánh (LIST/CREATE/EDIT)
- Quản lý nhân viên
...
- Modal đa bước với progress indicator
- Validation thời gian thực
- Feedback ngay lập tức
- Màu sắc phân loại theo trạng thái/rủi ro
...
Chức năng:
- Tất cả các module chính hoạt động ổn định
- Tính toán tự động (lãi, LTV, điểm tín dụng) chính xác
- Quy trình nghiệp vụ được tự động hóa

Hiệu năng:
- Tải trang nhanh (< 2 giây)
- Phản hồi tức thì cho các thao tác
- Xử lý tốt với dữ liệu mẫu

Trải nghiệm người dùng:
- Giao diện trực quan, dễ sử dụng
- Quy trình rõ ràng, không cần training nhiều
- Thông báo và hướng dẫn đầy đủ
...
- Ecosystem tài chính mở
 ...
 7.5. Khuyến nghị triển khai

 7.5.1. Cho đội ngũ kỹ thuật
