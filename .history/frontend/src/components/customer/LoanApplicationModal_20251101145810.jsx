      ...
      <form onSubmit={handleSubmit}>
        {step === 1 && (
          <div className="form-step">
            ...
            {/* Cảnh báo điều kiện vay không thế chấp */}
            {!selectedLoanType?.requireCollateral && formData.amount && formData.monthlyIncome && (
              <div className="unsecured-loan-check">
                {(() => {
                  const amount = parseFloat(formData.amount);
                  const income = parseFloat(formData.monthlyIncome);
                  const maxUnsecuredAmount = 200000000; // 200 triệu
                  const minIncome = 15000000; // 15 triệu
                  const maxDebtToIncome = income * 10;

                  const warnings = [];
                  let isEligible = true;

                  if (amount > maxUnsecuredAmount) {
                    warnings.push(`Số tiền vay vượt quá ${(maxUnsecuredAmount / 1000000).toFixed(0)} triệu (tối đa cho vay tín chấp)`);
                    isEligible = false;
                  }

                  if (income < minIncome) {
                    warnings.push(`Thu nhập thấp hơn ${(minIncome / 1000000).toFixed(0)} triệu/tháng (yêu cầu tối thiểu)`);
                    isEligible = false;
                  }

                  if (amount > maxDebtToIncome) {
                    warnings.push(`Số tiền vay vượt quá 10 lần thu nhập (tối đa ${(maxDebtToIncome / 1000000).toFixed(0)} triệu)`);
                    isEligible = false;
                  }
                  ...
                  return isEligible ? (
                    <div className="alert alert-success">
                      <Check size={20} />
                      <div>
                        <strong>✅ Đủ điều kiện sơ bộ</strong>
                        <div className="text-sm mt-1">
                          Hồ sơ của bạn sẽ được cán bộ tín dụng thẩm định thêm về:
                          <br/>• Điểm tín dụng (yêu cầu {'>='} 700 điểm, Hạng AA+)
                          <br/>• Lịch sử tín dụng (không có nợ xấu)
                          <br/>• Tỷ lệ nợ/thu nhập (tối đa 40%)
                        </div>
                      </div>
                    </div>
                  ) : (
                    <div className="alert alert-danger">
                      <AlertCircle size={20} />
                      <div>
                        <strong>❌ Chưa đủ điều kiện vay không thế chấp</strong>
                        <div className="text-sm mt-1">
                          {warnings.map((warning, idx) => (
                            <div key={idx}>• {warning}</div>
                          ))}
                          <div className="mt-2">
                            <strong>Gợi ý:</strong> Xem xét vay có thế chấp để được số tiền cao hơn và lãi suất thấp hơn.
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        <div className="flex gap-2 mt-4">
          ...
          {step < 3 ? (
            <button
              ...
              onClick={() => {
                if (step === 2 && selectedLoanType?.requireCollateral && !ltvResult) {
                  alert('Vui lòng tính toán LTV trước khi tiếp tục!');
                  return;
                }
                if (step === 2 && !selectedLoanType?.requireCollateral) {
                  setStep(3);
                  return;
                }
                setStep(step + 1);
