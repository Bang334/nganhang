...
const LoanApplication = () => {
  ...
  // Get customer ID and available assets
  ...
  const handleSelectAsset = (asset) => {
    ...
    // Map collateral_type_id to collateralType
    ...
    setFormData({
      ...formData,
      collateralType: collateralTypeMap[asset.collateral_type_id] || 'REAL_ESTATE',
          ...
          <div className="teller-card">
            ...
            <div className="teller-card-body">
              <div className="teller-alert info" style={{marginBottom: '1.5rem'}}>
                <TrendingUp size={18} />
                <div>
                  <strong>LTV (Loan-to-Value) tối đa cho {selectedLoanType.label}:</strong> {getMaxLtv()}%
                </div>
              </div>
                  ...
                  {availableAssets.length === 0 ? (
                    ...
                    <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1rem', marginBottom: '1.5rem'}}>
                      {availableAssets.map((asset) => {
                        ...
                        const assetType = collateralTypeMap[asset.collateral_type_id] || 'REAL_ESTATE';
                        
                        return <div
                          key={asset.collateral_id}
                          onClick={() => handleSelectAsset(asset)}
                          style={{
                            border: selectedAsset?.collateral_id === asset.collateral_id ? '2px solid #667eea' : '2px solid #e5e7eb',
                            borderRadius: '12px',
                            padding: '1.25rem',
                            cursor: 'pointer',
                            transition: 'all 0.3s ease',
                            position: 'relative',
                            background: selectedAsset?.collateral_id === asset.collateral_id ? 'linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%)' : 'white'
                          }}
                        >
                          <div style={{
                            width: '48px',
                          ...
                          <div style={{fontSize: '0.813rem', color: '#6b7280', marginBottom: '0.5rem', textTransform: 'uppercase'}}>
                            {asset.collateralTypeName || getAssetTypeLabel(assetType)}
                          </div>
                          {asset.location && (
                  ...
                  {selectedAsset && (
                    <div className="teller-info-box success">
                      ...
                      <div className="teller-info-row">
                        ...
                        <span className="teller-info-value">{selectedAsset.collateral_name}</span>
                      </div>
                      <div className="teller-info-row">
                        <span className="teller-info-label">Loại:</span>
                        <span className="teller-info-value">{selectedAsset.collateralTypeName}</span>
                      </div>
                      <div className="teller-info-row">
                        <span className="teller-info-label">Giá trị:</span>
                        <span className="teller-info-value">{formatCurrency(selectedAsset.appraised_value)}</span>
                      </div>
                      <div className="teller-info-row">
                        <span className="teller-info-label">Giấy tờ:</span>
                        <span className="teller-info-value">{selectedAsset.certificate_number}</span>
                      </div>
                    </div>
                  )}
                </>
          ...
          <div className="teller-card">
            ...
            <div className="teller-card-body">
              <div className="teller-info-box">
                ...
                <div className="teller-info-row">
                  <span className="teller-info-label">Hình thức:</span>
                  <span className="teller-info-value">
                    {formData.hasCollateral ? '🏠 Vay có thế chấp' : '💳 Vay tín chấp'}
                  </span>
                </div>
              </div>
              ...
              <div className="teller-actions">
                <button
                  type="button"
                  className="teller-btn teller-btn-secondary"
                  onClick={() => setStep(formData.hasCollateral ? 2 : 1)}
                >
                  Quay lại
                </button>
                <button 
