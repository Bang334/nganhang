...
const LoanApprovalModal = ({ isOpen, onClose, loanData }) => {
  ...
  const handleCreditScoring = () => {
    ...
    // ✅ KIỂM TRA VAY KHÔNG THẾ CHẤP
    if (!loanData.collateralType) {
      // Điều kiện vay không thế chấp (Unsecured Loan)
      const minCreditScore = 700; // Hạng AA trở lên
      const minMonthlyIncome = 15000000; // 15 triệu/tháng
      const maxDebtRatio = 40; // Tối đa 40%
      const maxLoanAmount = 200000000; // Tối đa 200 triệu
      const maxDebtToIncome = reviewData.monthlyIncome * 10; // <= 10 lần thu nhập

      const reasons = [];

      if (score < minCreditScore) {
        reasons.push(`❌ Credit Score ${score} < ${minCreditScore} (yêu cầu Hạng AA trở lên)`);
      }

      if (reviewData.monthlyIncome < minMonthlyIncome) {
        reasons.push(`❌ Thu nhập ${reviewData.monthlyIncome.toLocaleString()}đ < ${minMonthlyIncome.toLocaleString()}đ/tháng`);
      }

      if (reviewData.debtRatio > maxDebtRatio) {
        reasons.push(`❌ Tỷ lệ nợ/thu nhập ${reviewData.debtRatio}% > ${maxDebtRatio}%`);
      }

      if (reviewData.creditHistory !== 'GOOD') {
        reasons.push(`❌ Lịch sử tín dụng không tốt (yêu cầu: Không có nợ xấu)`);
      }

      if (loanData.loanAmount > maxLoanAmount) {
        reasons.push(`❌ Số tiền vay ${(loanData.loanAmount / 1000000).toFixed(0)} triệu > ${maxLoanAmount / 1000000} triệu`);
      }

      if (loanData.loanAmount > maxDebtToIncome) {
        reasons.push(`❌ Số tiền vay vượt quá 10 lần thu nhập (${(maxDebtToIncome / 1000000).toFixed(0)} triệu)`);
      }

      // Nếu không đủ điều kiện → TỪ CHỐI TỰ ĐỘNG
      if (reasons.length > 0) {
        alert(
          `❌ KHÔNG ĐỦ ĐIỀU KIỆN VAY KHÔNG THẾ CHẤP\n\n` +
          `Lý do:\n${reasons.join('\n')}\n\n` +
          `📋 YÊU CẦU VAY KHÔNG THẾ CHẤP:\n` +
          `✅ Credit Score >= ${minCreditScore} (Hạng AA+)\n` +
          `✅ Thu nhập >= ${(minMonthlyIncome / 1000000).toFixed(0)} triệu/tháng\n` +
          `✅ Tỷ lệ nợ/thu nhập <= ${maxDebtRatio}%\n` +
          `✅ Lịch sử tín dụng tốt (không nợ xấu)\n` +
          `✅ Số tiền vay <= ${maxLoanAmount / 1000000} triệu\n` +
        ...
        setFinalDecision({
          ...
          interestRate: 0,
          termMonths: 0,
          reason: `Không đủ điều kiện vay không thế chấp:\n${reasons.join('\n')}`,
          reviewedAt: new Date().toLocaleString('vi-VN'),
        });

        setStep(5); // Nhảy thẳng đến kết quả từ chối
        return;
      }
      ...
      alert(
        `✅ ĐỦ ĐIỀU KIỆN VAY KHÔNG THẾ CHẤP\n\n` +
        `Credit Score: ${score} (${grade.grade})\n` +
        `Thu nhập: ${(reviewData.monthlyIncome / 1000000).toFixed(1)} triệu/tháng\n` +
        `Tỷ lệ nợ: ${reviewData.debtRatio}%\n` +
        `Lịch sử tín dụng: ${reviewData.creditHistory}\n\n` +
        `Lãi suất áp dụng: ${grade.rate}%/năm (cao hơn vay có thế chấp)\n\n` +
        `→ Tiếp tục quyết định phê duyệt.`
      );
    }

    // Tiếp tục bước tiếp theo
    if (loanData.collateralType) {
      setStep(3); // Có thế chấp → Bước thẩm định tài sản
    } else {
      setStep(4); // Không thế chấp → Quyết định luôn
    }
  };

  const handleAppraisal = () => {
    const appraisalValue = reviewData.appraisalValue;
    if (!appraisalValue || appraisalValue <= 0) {
      alert('❌ Vui lòng nhập giá trị thẩm định!');
      return;
    }

    const ltv = calculateLTV(loanData.loanAmount, appraisalValue);
    const ltvRatio = parseFloat(ltv);
    ...
    // Kiểm tra LTV hợp lệ
    if (ltvRatio > reviewData.maxLtv) {
      // LTV không hợp lệ → TỪ CHỐI LUÔN
      alert(
        `❌ LTV KHÔNG HỢP LỆ - TỪ CHỐI KHOẢN VAY\n\n` +
        `LTV: ${ltvRatio}% > ${reviewData.maxLtv}% (Max)\n\n` +
        `Lý do: Giá trị thế chấp không đủ so với số tiền vay.\n` +
        `Khoản vay sẽ được TỪ CHỐI tự động.`
      );
      ...
      setReviewData({
        ...
        ltvRatio,
        decision: 'REJECTED',
        reason: `LTV vượt quá giới hạn (${ltvRatio}% > ${reviewData.maxLtv}%). Giá trị thế chấp không đủ.`,
      });
      ...
      {step === 2 && (
        <div className="form-step">
          ...
          <div className="form-group">
            ...
            <select
              className="input"
              value={reviewData.creditHistory}
              onChange={(e) => setReviewData({ ...reviewData, creditHistory: e.target.value })}
            >
              <option value="GOOD">Tốt (Không nợ xấu)</option>
              <option value="FAIR">Trung bình (Đã từng chậm trả)</option>
              <option value="POOR">Kém (Có nợ xấu)</option>
            </select>
          </div>
      ...
      {step === 3 && loanData.collateralType && (
        <div className="form-step">
          ...
          <div className="alert alert-warning" style={{marginTop: '1rem'}}>
            <strong>⚠️ Lưu ý:</strong> Nếu LTV vượt quá giới hạn cho phép ({reviewData.maxLtv}%), 
            khoản vay sẽ bị <strong>TỪ CHỐI TỰ ĐỘNG</strong>.
          </div>
