...
const LoanDetailModal = ({ isOpen, onClose, loanData }) => {
  ...
  const estimatedInterestRate = loanData.creditScore >= 700 ? 9.0 : loanData.creditScore >= 600 ? 10.0 : 12.0;
  
  // Calculate monthly payment safely
  let estimatedMonthlyPayment = 0;
  if (loanData.loanAmount && loanData.termMonths && loanData.termMonths > 0) {
    const monthlyRate = estimatedInterestRate / 100 / 12;
    const numerator = monthlyRate * Math.pow(1 + monthlyRate, loanData.termMonths);
    const denominator = Math.pow(1 + monthlyRate, loanData.termMonths) - 1;
    if (denominator > 0) {
      estimatedMonthlyPayment = loanData.loanAmount * (numerator / denominator);
    }
  }
          ...
          <div className="info-grid">
            ...
            <div className="info-item">
              ...
              <div className="value">
                ...
                {loanData.customerAddress || 'N/A'}
              </div>
            </div>
            <div className="info-item">
              <label>Nghề nghiệp</label>
              <div className="value">{loanData.customerOccupation || 'N/A'}</div>
            </div>
            <div className="info-item">
              <label>Thu nhập/tháng</label>
              <div className="value text-success font-semibold">
                {formatCurrency(loanData.customerMonthlyIncome || 0)}
              </div>
            </div>
            <div className="info-item">
              <label>Tỷ lệ nợ/thu nhập</label>
              <div className="value">
                <span className={(loanData.debtRatio || 0) > 50 ? 'text-danger' : 'text-success'}>
                  {loanData.debtRatio || 0}%
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="detail-section">
          ...
          <div className="credit-score-box">
            <div className="score-display">
              ...
              <div className="score-value">{loanData.creditScore || 0}</div>
              <div className="score-grade">Hạng {loanData.creditGrade || 'N/A'}</div>
            </div>
            <div className="score-details">
              ...
              <div className="score-item">
                ...
                <strong className={(loanData.employmentStatus || 'UNSTABLE') === 'STABLE' ? 'text-success' : 'text-danger'}>
                  {(loanData.employmentStatus || 'UNSTABLE') === 'STABLE' ? 'Ổn định' : 'Không ổn định'}
                </strong>
              </div>
            </div>
          </div>
        ...
        <div className="detail-section">
          ...
          <div className="info-grid">
            <div className="info-item">
              ...
              <div className="value font-semibold">{loanData.loanType || loanData.loanTypeName || 'N/A'}</div>
            </div>
            <div className="info-item">
              <label>Số tiền yêu cầu</label>
              <div className="value text-primary font-bold text-lg">
                {formatCurrency(loanData.loanAmount || loanData.loan_amount || 0)}
              </div>
            </div>
            <div className="info-item">
              <label>Thời hạn vay</label>
              <div className="value">
                {loanData.termMonths || 0} tháng ({loanData.termMonths ? Math.floor(loanData.termMonths / 12) : 0} năm)
              </div>
            </div>
            <div className="info-item">
              <label>Mục đích vay</label>
              <div className="value">{loanData.purpose || 'N/A'}</div>
            </div>
            <div className="info-item">
              <label>Ngày nộp hồ sơ</label>
              <div className="value">
                <Calendar size={14} className="inline mr-1" />
                {formatDateTime(loanData.submissionDate || loanData.application_date)}
              </div>
            </div>
            <div className="info-item">
