...
// Get verified collaterals (AVAILABLE or IN_USE)
export const getVerifiedCollaterals = (customerId) => {
  return collaterals
    .filter(c => c.customer_id === customerId && (c.status === 'AVAILABLE' || c.status === 'IN_USE'))
    .map(collateral => {
      const collateralType = collateralTypes.find(ct => ct.collateral_type_id === collateral.collateral_type_id);
      return {
        ...collateral,
  ...
  return loans
    ...
    .map(loan => {
      ...
      return {
        ...
        collateralName: collateral?.collateral_name,
        collateralValue: collateral?.appraised_value,
      };
    });
};
...
export const getCreditScoreData = (customerId) => {
  const scoreData = calculateCreditScore(customerId);
  if (!scoreData) return null;

  // Add score change (mock: random -20 to +30)
  const scoreChange = Math.floor(Math.random() * 51) - 20;

  // Add score history (mock: last 6 months)
  const scoreHistory = [];
  const months = ['T6', 'T7', 'T8', 'T9', 'T10', 'T11'];
  let baseScore = scoreData.creditScore - 50;
  for (let i = 0; i < 6; i++) {
    baseScore += Math.floor(Math.random() * 20);
    scoreHistory.push({
      month: months[i],
      score: Math.min(Math.max(baseScore, 300), 850),
    });
  }
...
export const getAllCustomersCreditScores = () => {
  return customers.map(customer => {
    const scoreData = calculateCreditScore(customer.customer_id);
    if (!scoreData) return null;
...
export const getVerifiedCollaterals = (customerId) => {
  ...
  return customerCollaterals.map(collateral => {
    ...
    if (isUsed) {
      status = 'IN_USE';
    } else if (collateral.verification_status !== 'VERIFIED') {
      status = 'PENDING';
    }
