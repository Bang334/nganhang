...
1	branch_id	INT (PK, AI)	Mã định danh chi nhánh
2	branch_code	VARCHAR(20)	Mã chi nhánh duy nhất
3	branch_name	VARCHAR(200)	Tên chi nhánh
4	address	VARCHAR(500)	Địa chỉ chi nhánh
5	phone	VARCHAR(20)	Số điện thoại
6	email	VARCHAR(100)	Email liên hệ
7	manager_id	INT (FK)	Quản lý chi nhánh (Employees)
8	status	ENUM('ACTIVE','INACTIVE')	Trạng thái hoạt động
9	created_at	TIMESTAMP	Thời điểm tạo

ssLưu thông tin các chi nhánh ngân hàng: mã, tên, địa chỉ, liên hệ, giám đốc.
Bảng: Roles (Vai trò)
STT	Tên cột	Kiểu dữ liệu	Mô tả
