...
-- Bảng Ngân hàng (để hỗ trợ chuyển khoản liên ngân hàng)
CREATE TABLE Banks (
...
CREATE TABLE Transactions (
    ...
    from_account_id INT,
    to_account_id INT,
    to_bank_id INT, -- Ngân hàng nhận (nếu chuyển khoản liên ngân hàng)
...
CREATE TABLE Loans (
    ...
    maturity_date DATE,
    status ENUM('PENDING', 'APPROVED', 'REJECTED', 'DISBURSED', 'ACTIVE', 'PAID_OFF', 'OVERDUE') DEFAULT 'PENDING',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (customer_id) REFERENCES Customers(customer_id),
    FOREIGN KEY (account_id) REFERENCES Accounts(account_id),
    FOREIGN KEY (loan_type_id) REFERENCES LoanTypes(loan_type_id),
